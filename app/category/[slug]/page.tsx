...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
    const { categoryData, loading, productData, getProductData } = useCategory()
    const { slug } = use(params)
    ...
    const showSkeleton = loading || (!dataReady && (!productData || productData.length === 0))
    ...
    const filterProducts = () => {
        ...
        switch (sortBy) {
            ...
            case "discount":
                ...
                break
            default:
                filtered.sort((a, b) => Number.parseInt(a?._id) - Number.parseInt(b?._id))
        }
    ...
    useEffect(() => {
        if (categoryData && categoryData.length > 0) {
            const category = categoryData.find((cat: any) => cat?._id === slug) as CategoryType | undefined
    ...
    }, [sortBy]) // only sort triggers automatic filtering
    ...
    return (
        <div className="w-full px-4 py-8 md:px-6 md:py-12">
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-[280px_1fr]">
                ...
                <div>
                    <div className="mb-6 flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Showing {filteredProducts?.length || 0} products</span>
                        <div className="flex items-center gap-2">
                            <span className="text-sm">Sort by:</span>
                            <Select
                                value={sortBy}
                        ...
                        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                            {filteredProducts.map((product, index) => (
                                <ProductCard
                                    ...
                                    product={product}
                                    getData={(noLoading) => getProductData(noLoading)}
                                />
                            ))}
                        </div>
                    ) : (
